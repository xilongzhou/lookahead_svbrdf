...
import numpy as np
import math
from . import des19_acquisitionScene as acquisitionScene
...
#Generate a random direction on the upper hemisphere with gaps on the top and bottom of Hemisphere. Equation is described in the Global Illumination Compendium (19a)
# output: [N,3]
def torch_generate_normalized_random_direction(nbRenderings, angle, lowEps = 0.001):
    r1 = torch.rand(nbRenderings, 1)*angle + lowEps
    r2 =  torch.rand(nbRenderings, 1)
    r = torch.sqrt(r1)
    phi = 2 * math.pi * r2
    #min alpha = atan(sqrt(1-r^2)/r)
    x = r * torch.cos(phi)
    ...
    finalVec = torch.cat((x, y, z), dim=-1) #Dimension here should be [nbRenderings, 3]
    return finalVec
...
def torch_DotProduct(tensorA, tensorB):
    temp = tensorA*tensorB
    # re = temp[...,0:1] + temp[...,1:2] + temp[...,2:3]
    # return re
    return torch.sum(temp, dim=-1, keepdim=True)
...
def torch_lampAttenuation_pbr(distance):
...
def generateInputRenderings(rendererInstance, material, batchSize, nbRenderings, surfaceArray, renderingScene, useAmbientLight, useAugmentationInRenderings = True):
    ...
    elif renderingScene == "coloviewlighthemisphere":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.mysettingviewlight(surfaceArray, batchSize, nbRenderings, useAugmentationInRenderings)
    else:
    ...
    surfaceArray = surfaceArray.unsqueeze(0).unsqueeze(0).repeat(batchSize,1,1,1,1)
    print('light pos: ',currentLightPos.shape)
    print('view pos: ',currentViewPos.shape)
    print('surfaceArray: ',surfaceArray.shape)
    ...
    if useAmbientLight:
        ...
        print('ambientLightPos pos: ',ambientLightPos.shape)
        wiAmbient = ambientLightPos - surfaceArray
        renderingAmbient = rendererInstance.torch_Render(material, wiAmbient, wo, None, currentLightPos = ambientLightPos, lossRendering = False, isAmbient = True, useAugmentation = useAugmentation)
   
    wi = currentLightPos - surfaceArray

    renderings = rendererInstance.torch_Render(material, wi, wo, currentConeTargetPos, currentLightPos = currentLightPos, lossRendering = False, isAmbient = False, useAugmentation = useAugmentation) # was currentConeTargetPos not None
    ...
    if useAmbientLight:
        ...
        renderingAmbient = torch.clamp(renderingAmbient, 0.0, 1.0)
        renderingAmbient = torch.pow(renderingAmbient, 0.4545)
        # print('renderingAmbient.dtype: ',renderingAmbient.dtype)
        # tf.summary.image("renderingAmbient", convert(renderingAmbient[0, :]), max_outputs=5)
    if useAugmentation:
        renderings = torch_addNoise(renderings)
    ...
    renderings = torch.clamp(renderings, 0.0, 1.0)
    renderings = torch.pow(renderings, 0.4545)

    return renderings,currentLightPos
...
def reshape_tensor_display(tensor, splitAmount, logAlbedo = False, axisToSplit = 3):
    ...
    if logAlbedo:
        ...
        tensors_list[-1] = tensors_list[-1]**(1/2.2)
        tensors_list[1] = tensors_list[1]**(1/2.2)
