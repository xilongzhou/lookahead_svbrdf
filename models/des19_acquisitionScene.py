...
range_light = 0.2 # the range of input light
...
# a scene with moving colocated camera and view position randomly sampled on the hemisphere
def mysettingviewlight(surfaceArray,batchSize,nbRenderings,useAugmentation):
    currentLightPos = torch_LightsHemisphere(batchSize,nbRenderings).cuda()
    currentViewPos = currentLightPos.clone()

    return currentLightPos, currentViewPos, None, surfaceArray, useAugmentation
...
## return [B,N,1,1,3]
def torch_LightsHemisphere(batch,nbRenderings):

    currentLightPos = torch.rand((1, 2), dtype=torch.float32)*1.5 - 0.75
    currentLightPos = torch.cat((currentLightPos,torch.ones(1,1)*lightDistance),dim=-1)

    if nbRenderings>1:
        currentLightPos2 = helpers.torch_generate_normalized_random_direction(nbRenderings - 1, angle=range_light, lowEps = minEps) * lightDistance #getting the pos and adding the multi light dim.
        currentLightPos = torch.cat((currentLightPos, currentLightPos2), dim = 0)
    #[N,3] --> [B,N,1,1,3]
    currentLightPos = currentLightPos.unsqueeze(1).unsqueeze(1).unsqueeze(0).repeat(batch,1,1,1,1)
...
def fullRandomLightsSurface(batchSize, nbRenderings):
    ...
    currentLightPos = torch.rand(batchSize, nbRenderings, 2)*2-1
    # currentLightPos = tf.random_uniform([batchSize, nbRenderings, 2], -1.0, 1.0, dtype=tf.float32)
    currentLightPos = torch.cat((currentLightPos, torch.ones(batchSize, nbRenderings, 1)* lightDistance), dim = -1)
    # currentLightPos = tf.expand_dims(tf.concat([currentLightPos, tf.ones([batchSize, nbRenderings, 1])* lightDistance], axis = -1), axis=-2)

    print('currentLightPos shape:', currentLightPos.shape)
    currentLightPos = simulateFlash(currentLightPos)
    # currentLightPos = tf.expand_dims(currentLightPos, axis=-2)
    # currentLightPos = tf.expand_dims(currentLightPos, axis=-2)
    return currentLightPos
...
def generateAmbientLight(currentLightPos, batchSize): #Maybe use the same cos distribution as in the other direction and pick a distance from a exp normal distribution
    ...
    ambiantDir = helpers.torch_generate_normalized_random_direction(1, angle = 0.8, lowEps = minEps) #Here remove angles below 25 ° from the surface
